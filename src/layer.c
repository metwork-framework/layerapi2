...
static const gchar *__layer_label_filename = NULL;
static const gchar *__layer_dependencies_filename = NULL;
static const gchar *__layer_conflicts_filename = NULL;
static const gchar *__layer_interactive_profile_filename = NULL;
static const gchar *__layer_interactive_unprofile_filename = NULL;
static const gchar *__layer_extra_env_filename = NULL;

const gchar *get_layer_label_filename()
{
    if (__layer_label_filename == NULL) {
        const gchar *tmp = g_getenv(LAYER_LABEL_FILENAME_ENV_VAR);
        if (tmp == NULL) {
            __layer_label_filename = LAYER_LABEL_DEFAULT_FILENAME;
        } else {
            __layer_label_filename = g_strdup(tmp);
        }
    }
    return __layer_label_filename;
}

const gchar *get_layer_dependencies_filename()
{
    if (__layer_dependencies_filename == NULL) {
        const gchar *tmp = g_getenv(LAYER_DEPENDENCIES_FILENAME_ENV_VAR);
        if (tmp == NULL) {
            __layer_dependencies_filename = LAYER_DEPENDENCIES_DEFAULT_FILENAME;
        } else {
            __layer_dependencies_filename = g_strdup(tmp);
        }
    }
    return __layer_dependencies_filename;
}

const gchar *get_layer_conflicts_filename()
{
    if (__layer_conflicts_filename == NULL) {
        const gchar *tmp = g_getenv(LAYER_CONFLICTS_FILENAME_ENV_VAR);
        if (tmp == NULL) {
            __layer_conflicts_filename = LAYER_CONFLICTS_DEFAULT_FILENAME;
        } else {
            __layer_conflicts_filename = g_strdup(tmp);
        }
    }
    return __layer_conflicts_filename;
}

const gchar *get_layer_interactive_profile_filename()
{
    if (__layer_interactive_profile_filename == NULL) {
        const gchar *tmp = g_getenv(LAYER_INTERACTIVE_PROFILE_FILENAME_ENV_VAR);
        if (tmp == NULL) {
            __layer_interactive_profile_filename = LAYER_INTERACTIVE_PROFILE_DEFAULT_FILENAME;
        } else {
            __layer_interactive_profile_filename = g_strdup(tmp);
        }
    }
    return __layer_interactive_profile_filename;
}

const gchar *get_layer_interactive_unprofile_filename()
{
    if (__layer_interactive_unprofile_filename == NULL) {
        const gchar *tmp = g_getenv(LAYER_INTERACTIVE_UNPROFILE_FILENAME_ENV_VAR);
        if (tmp == NULL) {
            __layer_interactive_unprofile_filename = LAYER_INTERACTIVE_UNPROFILE_DEFAULT_FILENAME;
        } else {
            __layer_interactive_unprofile_filename = g_strdup(tmp);
        }
    }
    return __layer_interactive_unprofile_filename;
}

const gchar *get_layer_extra_env_filename()
{
    if (__layer_extra_env_filename == NULL) {
        const gchar *tmp = g_getenv(LAYER_EXTRA_ENV_FILENAME_ENV_VAR);
        if (tmp == NULL) {
            __layer_extra_env_filename = LAYER_EXTRA_ENV_DEFAULT_FILENAME;
        } else {
            __layer_extra_env_filename = g_strdup(tmp);
        }
    }
    return __layer_extra_env_filename;
}
...
gchar *get_layer_env_var(const gchar *layer_home)
{
    ...
    res = g_strdup_printf("LAYERAPI2_LAYER_%s_LOADED", tmp2);
    g_free(tmp2);
    return res;
}
...
gboolean is_layer_installed(const gchar *layer_home)
{
    if (g_file_test(layer_home, G_FILE_TEST_IS_DIR)) {
        if (test_file_presence_in_directory(layer_home, get_layer_label_filename())) {
            return TRUE;
        }
    }
    return FALSE;
...
Layer *layer_new_from_home(const gchar *layer_home)
{
    ...
    out->label = read_file_in_directory(layer_home, get_layer_label_filename());
    out->dependencies = file_in_directory_to_gslist(layer_home, get_layer_dependencies_filename());
    out->conflicts = file_in_directory_to_gslist(layer_home, get_layer_conflicts_filename());
    return out;
}
...
void _layer_load(Layer *layer, gboolean force_prepend, GString **bash_cmds)
{
    ...
    conditional_prepend_env(layer->home, "node_modules", force_prepend, "NODE_PATH", bash_cmds);
    conditional_prepend_env(layer->home, "node_modules/.bin", force_prepend, "PATH", bash_cmds);
...
void _layer_unload(Layer *layer, GString **bash_cmds)
{
    ...
    conditional_remove_extra_env(layer->home, get_layer_extra_env_filename(), bash_cmds);
...
gboolean layer_load_recursive(Layer *layer, gboolean force_prepend, gboolean load_optional, GString **bash_cmds, gint recursion_level)
{
    ...
    while (deps_iterator != NULL) {
        gchar *label = (gchar*) deps_iterator->data;
        if (load_optional && ((label != NULL) && (label[0] == '-'))) {
            // optional dependency
