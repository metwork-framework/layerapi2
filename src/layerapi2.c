...
LayerApi2Layer *layerapi2_layer_load(const gchar *label_or_home, gboolean force_prepend, gboolean load_optional, GString **bash_cmds)
{
    LayerApi2Layer *res = NULL;
    Layer *layer = layer_new_from_label_or_home(label_or_home);
    if (layer != NULL) {
        gboolean tmp = layer_load(layer, force_prepend, load_optional, bash_cmds);
        if (tmp) {
